...
$vouchers = [];
$query = "SELECT * FROM ma_giam_gia WHERE trang_Thai = 'Đang hoạt động'";
...
if ($search !== '') {
    try {
        $stmt = $conn->prepare("SELECT * FROM san_pham WHERE (ten_San_Pham LIKE ? OR id_SP LIKE ?) AND trang_Thai = 'Còn hàng'");
        $searchParam = "%$search%";
        $stmt->execute([$searchParam, $searchParam]);
        $results = $stmt->fetchAll(PDO::FETCH_ASSOC);

        if (count($results) === 1) {
            // Chuyển đến trang chi tiết 1 sản phẩm
            $id_SP = $results[0]['id_SP'];
            ...
            // Không tìm thấy, lưu thông báo
            $_SESSION['search_message'] = "Không tìm thấy sản phẩm nào với từ khóa '$search'. Thử tìm 'áo nam' hoặc 'quần jeans' nhé!";
            header("Location: search_results.php?query=" . urlencode($search));
            exit();
        }
    } catch (Exception $e) {
...
// Phần còn lại của mã (vouchers, products, etc.) giữ nguyên...
// Lấy từ mã cũ của bạn
$vouchers = [];
$query = "SELECT * FROM ma_giam_gia WHERE trang_Thai = 'Đang hoạt động'";
$stmt = $conn->prepare($query);
$stmt->execute();
